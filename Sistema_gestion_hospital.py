...
def crearArchivoHistoriaClinica(documentoPacienteNuevo: str) -> bool:
    ...
    try:
        ...
        archivoHistoriaClinica.write("Fecha ingreso:" + datetime.now().strftime("%d/%m/%Y") + "\n")
...
def generarFacturaPaciente(documentoPacienteFactura: str, valorTotalFactura: int, cantidadDiasFactura: int) -> bool:
    ...
    try:
        ...
        archivoFacturaPaciente.write("Dias hospitalizacion:" + str(cantidadDiasFactura) + "\n")
...
def facturarPacienteHospital(documentoPacienteFacturacion: str, cantidadDiasHospitalizacion: int, matrizHabitacionesTipoA: np.ndarray, matrizHabitacionesTipoB: np.ndarray) -> int | bool:
    ...
    try:
        ...
        generarFacturaPaciente(documentoPacienteFacturacion, valorTotalFactura, cantidadDiasHospitalizacion)
    ...
    try:
        ...
        if os.path.exists(carpetaArchivo):
            ...
            for nombreArchivoHistoria in listaArchivosHistoria:
                ...
                if nombreArchivoHistoria.startswith("factura_") and nombreArchivoHistoria.endswith(".txt"):
                    ...
                    try:
                        ...
                        documentoFactura = nombreArchivoHistoria.replace("factura_", "").replace(".txt", "")

                        for lineaHistoriaReporte in archivoHistoriaReporte:

                            if lineaHistoriaReporte.startswith("Dias hospitalizacion:"):

                                diasEstadiaFactura = lineaHistoriaReporte.strip().split(":", 1)[1].strip()

                        archivoHistoriaReporte.close()

                        if diasEstadiaFactura != "":

                            totalDiasEstadia += int(diasEstadiaFactura)

                            cantidadPacientesAlta += 1

                        rutaArchivoHistoriaClinica = carpetaArchivo + "historia_" + documentoFactura + ".txt"

                        if os.path.exists(rutaArchivoHistoriaClinica):

                            archivoHistoriaClinicaReporte = open(rutaArchivoHistoriaClinica, "r", encoding="utf-8")

                            for lineaHistoriaReporte in archivoHistoriaClinicaReporte:
                            ...
                            archivoHistoriaClinicaReporte.close()
            ...
            while sesionActiva == True:
                ...
                if rolUsuarioLogueado == "admin":
                    ...
                    if opcionMenuSistema == "1":
                        ...
                        for filaImpresionB in range(20):
                    ...
                    if opcionMenuSistema == "1":
                        ...
                        if documentoPacienteActualizar != "":
